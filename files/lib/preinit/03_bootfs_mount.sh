...
bootfs_mount() {
  ...
  if [ $? -eq 0 ]; then
    ...
    if [ ! -f "$BOOTFS_TRIG" ]; then
      printf "preinit: moving backup to overlay file system" > /dev/kmsg
      ...
      if [ $? -eq 0 ]; then
        printf "preinit: successfully mounted the new overlay" > /dev/kmsg
        rm -rf $OVERLAYFS
        if [ -f "$BOOTFS_TARZ" ]; then
          printf "preinit: found the sysupgrade backup file!" > /dev/kmsg
          cp -ar $BOOTFS_TARZ $OVERLAYFS
        else
          printf "preinit: sysupgrade backup file not found!" > /dev/kmsg
        fi
        rm -rf $BOOTFS
